...
plt.tight_layout()
plt.show()
# Выводы: Наибольшую корреляцию с `default_flg` имеют `Score_bki`,
# `appl_rej_cnt` и `out_request_cnt`.
...
# Выводы: Категориальные признаки имеют низкую мощность,
# поэтому One-Hot Encoding является подходящей стратегией.
...
# Получение имен сгенерированных признаков для анализа
...
def evaluate_model(model, X_test_data, y_test_data, model_name="Модель"):
    ...
    Args:
        model: Обученная модель Sklearn-совместимого интерфейса.
        X_test_data: Тестовая матрица признаков.
        y_test_data: Тестовый вектор целевой переменной.
        model_name: Название модели для вывода в отчете.
    
    Returns:
        Кортеж (f1_score, y_pred, y_pred_proba).
    """
...
# Выводы: Логистическая регрессия показывает приемлемое качество и может служить хорошим baseline.
...
# Выводы: XGBoost показывает значительно лучшие результаты по сравнению с логистической регрессией.
...
# Выводы: CatBoost показывает результаты, сопоставимые с XGBoost.
...
# Выводы: Random Forest с параметром class_weight также показывает хорошие результаты.
...
if hasattr(best_model_data["model"], 'feature_importances_'):
    ...
    plt.ylabel("Признак")
    plt.show()
    # Выводы: Наиболее влиятельными признаками являются `Score_bki`,
    # `appl_rej_cnt` и `out_request_cnt`, что согласуется с EDA.
...
Лучшие результаты продемонстрировала модель CatBoost с F1-score = 0.53.
Анализ важности признаков показал, что ключевую роль играют `Score_bki`, `appl_rej_cnt` и `out_request_cnt`.

Рекомендации:
Для внедрения в продуктовый pipeline рекомендуется использовать модель CatBoost с гиперпараметрами,
