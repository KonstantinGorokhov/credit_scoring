...
with st.form("credit_form"):
    ...
    car_type_ru = st.selectbox("Автомобиль — иномарка", ["Да", "Нет"])
    good_work_ru = st.selectbox("Хорошее место работы", ["Да", "Нет"])

    income = st.number_input("Ежемесячный доход (₽)", 0, 10_000_000, 50_000)
    air_ru = st.selectbox("Наличие загранпаспорта", ["Да", "Нет"])
    ...
    # ======================
    if mode == "Ручной ввод":
        appl_rej_cnt = st.slider("Количество отказов ранее", 0, 5, 1)
        Score_bki = st.slider("Скор БКИ", -3.0, 3.0, -1.5, step=0.1)
        out_request_cnt = st.slider("Запросов в БКИ", 0, 5, 1)
        region_rating = st.selectbox("Рейтинг региона", [20, 30, 40, 50, 60, 70, 80])
        ...
        first_time_cd = st.selectbox("Давность кредитной истории", [1, 2, 3, 4, 5])
